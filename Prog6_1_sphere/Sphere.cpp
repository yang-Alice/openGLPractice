...
void Sphere::init(int prec) {
	...
	for (int i = 0; i <= prec; i++) {
		for (int j = 0; j <= prec; j++) {
			//float y = (float)cos(toRadians(180.0f - i * 180.0f / prec)); //y的值从 180°到0°
			//float x = -(float)cos(toRadians(j*360.0f / prec))*(float)abs(cos(asin(y)));//x的值从 0°到360°
			//float z = (float)sin(toRadians(j*360.0f / (float)(prec)))*(float)abs(cos(asin(y)));//z值从 0°到360°
			float y = (float)cos(toRadians(180.0f - i * 180.0f / prec));
			float sin_theta = (float)sqrt(1 - y * y); // 计算 sin(θ)
